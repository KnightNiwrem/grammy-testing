...
- **Membership sync** mirrors verified Bot API semantics:
  ...
  - `promoteChatMember` sets `'administrator'` with the granted rights (`can_manage_chat`
    ...
    membership.
  - Creators are never mutated (real Telegram rejects moderating the owner), bans,
    unbans, and restricts never rewrite an administrator record (Telegram requires
    demotion first), and calls targeting user IDs not minted by the orchestrator are
    logged without state sync.
